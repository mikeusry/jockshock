 ...
 *
 * The browser posts here, not to Nexus. Same split as southland-platform:
 *   - the customer's IP reaches Nexus as the forwarded header rather than
 *     Vercel's edge, so rate limiting counts real people;
 ...
 * Do not put `?review=` (a single-use purchase token) on a GroupMe / shared
 * link — first submit burns it. The open form is the right tool for that ask.
 */
...
const NEXUS_BASE =
  import.meta.env.NEXUS_API_BASE || "https://nexus.southlandorganics.com";
