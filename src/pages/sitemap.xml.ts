...
// Build-time map of "route file -> last commit date", generated by
// scripts/gen-page-dates.mjs via the prebuild step. Empty when git history is
// unavailable, in which case <lastmod> is omitted rather than guessed.
...
export const GET: APIRoute = () => {
  // route -> lastmod, resolved from the glob key ("./learn/x/index.astro" ->
  // "learn/x/index.astro"). Built alongside the route list so each URL keeps
  // its own date; a route with no git history simply has no entry.
  const lastmodByRoute = new Map<string, string>();
  for (const key of Object.keys(PAGE_MODULES)) {
    const route = globKeyToRoute(key);
    if (!route) continue;
    const date = DATES[key.replace(/^\.\//, "")];
    if (date) lastmodByRoute.set(route, date);
  }
  ...
  const urls = routes
    .map((route) => {
      ...
      // Omit <lastmod> entirely when unknown. An absent lastmod is a non-signal;
      // a fabricated one tells Google a page changed when it did not.
      return `  <url>
